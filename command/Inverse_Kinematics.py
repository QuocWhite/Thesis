...
def transform_matrix_to_pose_vector(transform_matrix):
    ...
    pose_vector[0][0] = transform_matrix[0, 3]  # x
    pose_vector[1][0] = transform_matrix[1, 3]  # y
    pose_vector[2][0] = transform_matrix[2, 3]  # z
    pose_vector[3][0] = euler_angles[0]         # yaw
    pose_vector[4][0] = euler_angles[1]         # pitch
    pose_vector[5][0] = euler_angles[2]         # roll
    return pose_vector
...
def compute_forward_kinematics(joint_angles_deg):
    ...
    transform = np.eye(4)
    for i in range(6):
        theta, d, a, alpha = dh_parameters[i, 1], dh_parameters[i,
                                                                0], dh_parameters[i, 2], dh_parameters[i, 3]
...
def inverse_kinematics_solver(target_pose_vector, initial_joint_angles_deg, max_iterations=500, tolerance=1e-2, learning_rate=0.5):
