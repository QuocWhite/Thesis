...
def capture_and_detect(camera_id=0):
    """Capture a single frame from webcam, run detection, return detections."""
    yolo = YOLO2D(
        model_path="model_data/yolo_weights.h5",
        anchors_path="model_data/yolo_anchors.txt",
        classes_path="model_data/coco_classes.txt",
    ...
    _, detections = yolo.detect_image_bgr(frame)
    return detections


def handler():
    """Interactive webcam loop for testing YOLO detection visually."""
    yolo = YOLO2D(
        model_path="model_data/yolo_weights.h5",
        anchors_path="model_data/yolo_anchors.txt",
        classes_path="model_data/coco_classes.txt",
