...
calib_data_path = "/mnt/mydrive/ARMv6/calib_data/MultiMatrix.npz"
...
if __name__ == "__main__":
    # Change this to the camera index you want to kill processes for
    camera_index_to_kill = 0
...
def prespective_transforme(frame, cen, cor):
    pts_original = np.float32(
        [cen["cen1"], cen["cen2"], cen["cen4"], cen["cen3"]])
...
def redist(corners1, corners2):
    rvec1, tvec1, _ = aruco.estimatePoseSingleMarkers(
        corners1, MARKER_SIZE, cam_mat, dist_coef)
    rvec2, tvec2, _ = aruco.estimatePoseSingleMarkers(
        corners2, MARKER_SIZE, cam_mat, dist_coef)
